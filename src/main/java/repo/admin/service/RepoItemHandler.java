...
public class RepoItemHandler {
    ...
    public void setValue(RepoItem repoItem, String attributeName, Object attributeValue) {
        Object oldValue = repoItem.getAttribute(attributeName).getValue();
        repoItem.setValue(attributeName, attributeValue);

        messagingService.sendChangeEvent(repoItem.getId(), attributeName, oldValue, attributeValue);
        assertConsistencyIfActive(repoItem);
    }

    public ERepoItemState getState(RepoItem repoItem) {
        ERepoItemState state = ERepoItemState.valueOf(getValues(repoItem, Constants.STATE_ATTRIBUTE_NAME).iterator().next().getValue().toString());
        return state;
    }
